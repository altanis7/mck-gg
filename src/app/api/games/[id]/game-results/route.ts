...
export async function GET(
  ...
  try {
    ...
    if (error) {
      return NextResponse.json<GameResultsResponse>(
        { success: false, error: error.message },
        { status: 500 }
      );
    }

    return NextResponse.json<GameResultsResponse>({ success: true, data });
  } catch (error) {
    return NextResponse.json<GameResultsResponse>(
      {
        success: false,
        error: error instanceof Error ? error.message : "게임 결과 조회 실패",
      },
      { status: 500 }
    );
  }
...
export async function POST(
  ...
  try {
    ...
    if (!token || !verifyToken(token)) {
      return NextResponse.json<GameResultResponse>(
    ...
    // position 검증
    if (!["top", "jungle", "mid", "adc", "support"].includes(body.position)) {
      return NextResponse.json<GameResultResponse>(
        {
          success: false,
      ...
      .insert([
        {
          ...
          damage_taken: body.damage_taken || 0,
          ...
          vision_score: body.vision_score || 0,
          wards_placed: body.wards_placed || 0,
          wards_destroyed: body.wards_destroyed || 0,
    ...
    return NextResponse.json<GameResultResponse>(
      {
        success: false,
        error: error instanceof Error ? error.message : "게임 결과 생성 실패",
      },
      { status: 500 }
    );
  }
